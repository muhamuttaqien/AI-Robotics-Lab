...
public:
    ...
    rate_(10)
  {
    ...
    n.param<std::string>("waypointsfile",
        filename,
        ros::package::getPath("cirkit_waypoint_navigator") + "/waypoints/room_new.csv"); // FIXME: Don't find!

    //n.param("dist_thres_to_target_object", dist_thres_to_target_object_, 1.8);
    n.param("dist_thres_to_target_object", dist_thres_to_target_object_, 0.1);
  ...
  int readWaypoint(std::string filename) {
    ...
    while (ifs.good()) {
      ...
      if (data.size() != rows_num) {
        ...
        //次のwaypointへ行く閾値がgeneratorの方で3に設定してあるので割る
  ...
  geometry_msgs::Pose getRobotCurrentPosition() {
    ...
    try {
      ...
      // laser scan matcher navigationの時は以下に設定
