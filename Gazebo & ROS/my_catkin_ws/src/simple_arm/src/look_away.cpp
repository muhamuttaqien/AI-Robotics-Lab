...
// Define global vector of joints last position, moving state of the arm, and
// the client that can request services
std::vector<double> joints_last_position{0, 0};
bool moving_state = false;
ros::ServiceClient client;

// This function calls the safe_move service to safely move the arm to the
// center position
...
// This callback function continuously executes and reads the arm joint angles
// position
void joint_states_callback(const sensor_msgs::JointState js) {
  ...
  // Check if the arm is moving by comparing its current joints position to its
  // latest
...
void look_away_callback(const sensor_msgs::Image img) {
  ...
  // If the image is uniform and the arm is not moving, move the arm to the
  // center
...
int main(int argc, char **argv) {
  ...
  // inside the joint_states_callback function
  ros::Subscriber sub1 =
